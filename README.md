...
**Estimator cards.** Any technician tagged `Estimator` (office staff who write estimates rather than do field work — `isEstimator`/`renderEstimatorCard` in `app.js`) gets an entirely different card instead of the standard one: the usual field-tech tiles (Jobs, Revenue, Leads, RCC sold, IFO, Accessory sold, ...) would all read zero for them, since those all derive from jobs they're never assigned to. Their card shows five tiles instead — Estimates given, Estimates approved, Approved this period (all as above), **Closing %** (Estimates approved ÷ Estimates given for the selected period — not mixed with the separate approval-date "Approved this period" number, since a rate combining two different date scopes wouldn't mean anything), and **Revenue accepted** (sum of `approved_amount` — see below — across that period's approved estimates). The expandable detail list shows their individual estimates (date, approved/pending, customer, and amount if approved) instead of a job list, since a pure estimator has no jobs to show.
