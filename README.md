...
**Cancelled calls** (a 5th tile in the Company summary row) shows the count and rate of `user canceled`/`pro canceled` jobs in the current filtered/period view — `computeCancellationStats` in `shared.js`. This is the one place canceled jobs are counted rather than excluded.

**Lead source performance** groups the same jobs by `lead_source` (a field Housecall Pro sets on the job, distinct from business unit/technician), using the same scorecard component and tag-based metrics as departments. Sources with $0 revenue in view are dropped entirely — not useful to show — and jobs with no lead source land in an "Unknown source" bucket (muted gray, not a real category, same treatment as "No business unit set"). Ranked by revenue, the top 6 render full-size like department cards; the rest sit behind a "+N more lead sources" toggle in a smaller multi-column grid. Both of these are **Company Metrics (`admin.html`) only** — the technician view doesn't show them.
