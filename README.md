...
**Lead source performance** groups the same jobs by `lead_source` (a field Housecall Pro sets on the job, distinct from business unit/technician), using the same tag-based metrics as departments. Sources with $0 revenue in view are dropped entirely — not useful to show — and jobs with no lead source land in an "Unknown source" bucket (muted gray, not a real category, same treatment as "No business unit set"). Ranked by revenue, the top 6 render as **equal-width segments of a single bar** (`renderLeadSourceBar` in `admin.js`) rather than separate cards — lead source revenue is typically heavily skewed (one channel can outweigh another 1000:1), so segment width is *not* proportional to revenue; a strictly proportional segment for a small source would be too thin to hold a legible label. Equal width keeps every featured source readable; left-to-right ranking (highest revenue first) still conveys relative importance. Sources past the top 6 sit behind a "+N more lead sources" toggle, shown as a smaller multi-column card grid. Both cancellation-per-department and lead source performance are **Company Metrics (`admin.html`) only** — the technician view doesn't show them.
