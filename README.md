...
**Metrics:** every technician tile — the featured card, a list row, or a BU screen's totals row (see below) — shows the same 8: Revenue, Avg ticket, Completion, Jobs, Leads, Leads sold, IFO, Accessory sold (`metricTiles` in `tv.js`, drawing on the same `computeScorecardStats` fields the desktop scorecards use).
...
**BU screens get an extra totals row** at the bottom of the list — the roster's combined stats for that business unit, computed directly from the BU's jobs (not summed from the rows above), unsplit revenue, same convention `admin.html`'s department cards use (`buildBuTotals` in `tv.js`). The three tag-only screens don't have an equivalent single business unit to total, so they don't get this row. A BU screen's header and featured-card rank subtitle both show the business unit's exact name as synced from Housecall Pro (e.g. "30 HVAC SERVICE"), read live off a matching job rather than hardcoded, so it always matches what `admin.html` shows for the same BU (`businessUnitLabelForCode` in `tv.js`).
