...
`tv.html` / `tv.js` / `tv.css` are a third, separate page — not a mode of the interactive dashboard — built for an unattended shop-floor TV. No filter bar, no nav, nothing clickable; it just shows one department's leaderboard and refreshes itself. Not linked from the other pages (there's no reason a desk-side viewer would navigate to it); each physical TV is just pointed at its own URL.

**Required:** `?dept=` — one of `Plumbing Service`, `Plumbing Installation`, `HVAC Service`, `HVAC Installation`, `Office` (exact match, case-sensitive). Anything else shows an on-screen error listing the valid values instead of a blank page. Example: `tv.html?dept=Plumbing%20Service`.

**Department mapping** (`departmentOf` in `tv.js`): the four field departments are real employee tags already used elsewhere (the technician-view quick-filter buttons). There's no equivalent "Office" tag, so `Office` is a catch-all — anyone not carrying one of the four field tags — rather than a hardcoded list of office-specific tags (`Customer Service Specialist`, `Estimator`, `Office Manager`, ...) that could go stale as new ones appear. `EXCLUDED_TECH_IDS` drops three accounts that showed up in that catch-all but aren't real people (Marketing Department, Urgency Plumbing, Urgency HVAC — dispatch/system accounts). One known consequence of the catch-all: a technician with no department tag at all lands on the Office TV regardless of their actual role (e.g. a field tech missing a tag) — a real gap in Housecall Pro's tagging, not a bug here, but worth knowing about if an Office TV shows someone unexpected.

**Optional params:** `?period=` (same values as the rest of the app — `today`/`week`/`lastweek`/`month`/`lastmonth`/`ytd`; defaults to `month`).

**Metrics:** every technician tile — the featured card, a list row, or a BU section's totals row (see below) — shows the same 8: Revenue, Avg ticket, Completion, Jobs, Leads, Leads sold, IFO, Accessory sold (`metricTiles` in `tv.js`, drawing on the same `computeScorecardStats` fields the desktop scorecards use).

**Ranking:** every technician in the department is ranked by split Revenue (`computeScorecardStats(jobs, { splitRevenue: true })`, same metric as the technician view's "Revenue (split)") for the selected period, highest first — including $0 techs, ranked last, so the full roster is always visible rather than only whoever has activity. Three of the five screens (HVAC Installation, Plumbing Installation, Office) show one large "featured" card for the current #1 (big photo, name, rank, the 8-tile grid), with everyone else as a smaller ranked row in a list beside it — #1 always holds the featured spot; there's no timer or rotation, it just re-sorts on every data refresh.

**HVAC Service and Plumbing Service are split by business unit, not shown as one blended list.** Techs tagged one of these two departments actually work across two distinct business units in Housecall Pro (HVAC Service → BU 30 and 40; Plumbing Service → BU 70 and 80), so each of these two screens renders one section per BU instead of the single featured-card layout — a compact ranked list plus a totals row, stacked vertically (`DEPT_BU_SPLIT`/`buildBuRanked`/`renderBuSection` in `tv.js`). A tech can appear in both of a screen's sections at a different rank, since each section's stats are scoped to only that BU's jobs (e.g. someone who's #1 in BU 30 revenue might be #5 in BU 40); the full department roster still shows in every section even at $0, same principle as the unsplit screens. Each section's totals row aggregates that BU's jobs directly (not summed from the rows above it), unsplit revenue — same convention `admin.html`'s department cards already use. A section's header is the business unit's exact name as synced from Housecall Pro (e.g. "30 HVAC SERVICE"), read live off a matching job rather than hardcoded, so it always matches what `admin.html` shows for the same BU.
