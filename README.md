...
- `docs/` — static dashboard (plain HTML/CSS/JS, no build step) served by GitHub Pages. `index.html`/`app.js` and `admin.html`/`admin.js` share `shared.js` (data formatting, scorecard rendering, period math); `tv.html`/`tv.js` reuses `shared.js`'s data/compute helpers but has its own rendering and its own stylesheet (`tv.css`) since the TV layout has nothing in common visually with the other two. Three pages total:
  ...
  **Completion rate excludes jobs that haven't started yet.** A job merely `scheduled` (or still `needs scheduling`) hasn't had any chance to be completed — counting it would make a tech/department/BU look worse for simply having a full week ahead of them. `NOT_YET_STARTED_STATUSES` in `shared.js` drops those from completion rate's denominator entirely (in both `computeStats` and `computeScorecardStats`, so this applies everywhere completion shows — team/company summary, every scorecard, and the TV kiosk): a job only counts once it's `in progress` or complete. Every other metric (Jobs, Revenue, etc.) is unaffected and still counts a scheduled job normally.
...
**Metrics:** every technician tile — the featured card or a list row — shows the same 8: Revenue, Avg ticket, Completion, Jobs, Leads, Leads sold, IFO, Accessory sold (`metricTiles` in `tv.js`, drawing on the same `computeScorecardStats` fields the desktop scorecards use).
