...
**Estimator cards.** Any technician tagged `Estimator` (office staff who write estimates rather than do field work — `isEstimator`/`renderEstimatorCard` in `app.js`) gets an entirely different card instead of the standard one: the usual field-tech tiles (Jobs, Revenue, Leads, RCC sold, IFO, Accessory sold, ...) would all read zero for them, since those all derive from jobs they're never assigned to. Their card shows five tiles instead — Estimates given, Estimates approved, Approved this period (all as above), **Closing %**, and **Revenue accepted** (sum of `approved_amount` — see below — across this period's given-and-approved estimates). Closing % is a deliberately mixed-scope ratio — Approved this period (approval-date scoped) ÷ Estimates given (creation-date scoped) — so an estimator's rate credits them for closing older proposals this period rather than only ever measuring what they gave this exact period; it can exceed 100% in a period where more old proposals close than new ones are given. The expandable detail list shows their individual estimates (date, approved/pending, customer, and amount if approved) instead of a job list, since a pure estimator has no jobs to show.
