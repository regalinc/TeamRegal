...
**Optional params:** `?period=` (same values as the rest of the app — `today`/`week`/`lastweek`/`month`/`lastmonth`/`ytd`; defaults to `month`) and `?rotate=` (seconds between the featured card rotating to the next technician; defaults to `8`).

**Ranking:** every technician in the department is ranked by split Revenue (`computeScorecardStats(jobs, { splitRevenue: true })`, same metric as the technician view's "Revenue (split)") for the selected period, highest first — including $0 techs, ranked last, so the full roster is always visible rather than only whoever has activity. One large "featured" card shows the top-ranked-and-rotating technician (big photo, name, rank, a 4-tile KPI grid); everyone else renders as a smaller ranked row in a list beside it, with the currently-featured one outlined.
