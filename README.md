...
**Estimator cards.** Any technician tagged `Estimator` (office staff who write estimates rather than do field work — `isEstimator`/`renderEstimatorCard` in `app.js`) gets an entirely different card instead of the standard one: the usual field-tech tiles (Jobs, Revenue, Leads, RCC sold, IFO, Accessory sold, ...) would all read zero for them, since those all derive from jobs they're never assigned to. Their card shows five tiles instead — Estimates given, Estimates approved (both creation-date scoped, as above), **Approved this period** and **Revenue accepted** (both approval-date scoped — Revenue accepted sums `approved_amount`, see below, across every estimate approved within the period regardless of when it was given), and **Closing %** (Approved this period ÷ Estimates given). Closing % and Revenue accepted are deliberately mixed-scope: an estimate given last period but approved this one now counts toward both, so an estimator's numbers credit them for closing older proposals rather than only ever measuring what they gave in the exact selected period — Closing % can exceed 100% in a period where more old proposals close than new ones are given. The expandable detail list shows the union of this period's given estimates and this period's approvals (date, approved/pending, customer, and amount if approved), not just one or the other, so every estimate contributing to any tile above is visible in it.
