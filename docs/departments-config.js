...
const DEPARTMENTS = {
  30: {
    ...
    hcp: [
      ...
      // Estimates approved ÷ estimates given, scoped to this department
      // via the estimate's own estimate_fields.business_unit (see
      // toPublicEstimate, scripts/sync.js) rather than the assigned
      // tech's employee tag — HVAC Service techs write estimates for both
      // BU 30 and BU 40 work, so a tech-tag-based split couldn't tell the
      // two apart; the estimate's own Business Unit field can. Target is
      // ≥ 50% for all four departments that carry this tile (30/40/70/80),
      // confirmed by the user together as one number.
      { key: "estimateClosingRate", label: "Estimate closing %", type: "pct", target: { goal: 0.5, direction: "min" } },
    ],
    pnl: [
    ...
    // already defined in hcp/pnl/manual above (kept intact so the matrix
    // page, which reads pnl + the revenue-per-X manual keys, is
    // unaffected); `label`/`target` here override the base metric's for
    // this page's display without touching the shared definition. See
    // renderScorecardSection in company-scorecard.js. Departments without
    // a `scorecard` array (BU 10/50) still render the old 3 sections.
    scorecard: [
      ...
      // DFO here vs. "$0 Call" on the hcp chart / admin.html — same
      // underlying metric (jobs tagged IFO ÷ total jobs), relabeled, and
      ...
      { source: "hcp", key: "leadTurnover", label: "Lead turnover (1 per 8-12 calls)" },
      { source: "hcp", key: "avgTicket", label: "Avg ticket (no sales)" },
      { source: "hcp", key: "accessorySold", label: "Accessory sales (1 per ~8 calls)" },
      { source: "manual", key: "reviewsGenerated", label: "Reviews generated by technicians" },
  ...
  40: {
    ...
    manual: [
      ...
      // Added for the revamped `scorecard` list below — the new-vs-renewal
      // club split Housecall Pro doesn't expose, so both are hand-entered
      // (from wherever the club program tracks them). Targets: new-customer
    ...
    // the flat club-conversion tile in favor of the hand-entered
    // new-vs-renewal split above.
    scorecard: [
      { source: "hcp", key: "avgTicket", label: "Avg ticket" },
      { source: "manual", key: "newCustClubConversion", label: "New-customer club conversion" },
  ...
  80: {
    ...
    // Curated view for company-scorecard.html only (see BU 30's scorecard
    // comment). BU 80's revamped list is the shortest — no P&L section at
